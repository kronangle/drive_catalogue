...
//	=====================================================================================
	 ...
	 * Executes the OAuth flow in a background thread to keep the UI responsive.
	 */
	@FXML
	private void loginButtonClicked() {
		App.showNotification("Logging in to Google Account...");
		logger.info("Login initiated by user.");

		// 1. Run logic in Background Thread (VIP Lane)
		// This prevents the application window from freezing while the browser opens.
		CompletableFuture.runAsync(() -> {
			try {
				...
				Platform.runLater(() -> {
					logger.info("Login successful. Switching to Home.");
					JavaFXTools.loadScene(FxmlView.HOME);
					App.bringStageToFront();
				});

			} catch (IOException | GeneralSecurityException e) {
				// 4. Failure -> Notify User (Must run on UI Thread)
				Platform.runLater(() -> {
					logger.warn("Login failed", e);
					App.showNotification("Login failed - " + e.getMessage());
				});
			} catch (Exception e) {
				// Catch-all for unexpected runtime errors
				Platform.runLater(() -> {
					logger.error("Unexpected error during login", e);
					App.showNotification("An unexpected error occurred.");
				});
			}
		});
	}
